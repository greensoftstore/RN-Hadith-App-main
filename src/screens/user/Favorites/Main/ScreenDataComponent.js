...
// Components
// ===================================================================
import { MainBackgroundWidthShadow, ListEmptyComponent, SpinnerCustom, Header, ModalAddFavourite, BottomTabBar, FullScreenLoader } from 'components'
...
function ScreenDataComponent({ navigation, screenSettings }) {
    ...
    const getData = useCallback(async () => {
        let res = await Api.getData(userId, 15, state.lastItem)
    ...
    const getMoreItems = useCallback(async () => {
        let res = await Api.getMoreData(userId, 15, state.lastItem)
        ...
        <>
            <View style={{ width: '100%', flex: 1, overflow: 'hidden' }}>
                ...
                <Header
                    ...
                    displayAddFavourite={true}
                    onPressAddFavourite={onPressSettings}
                />
