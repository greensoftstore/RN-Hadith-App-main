...
// ===================================================================
import { isToday, isYesterday, } from 'utilities/dataFormatting/date'
...
const SingleCollection = ({ t, navigation, item, selectedItem, displaySkeletonLoader }) => {
    ...
    let lastEdited = item?.lastEdited || null
    let note = item?.note || ''
    let content = item?.content || ''

    let lastEditedtext = lastEdited != null ? isToday(lastEdited?._seconds, true) ? 'danas' : isYesterday(lastEdited?._seconds, true) ? 'jučer' : moment.unix(lastEdited?._seconds).locale('bs').format('DD. MMM, YYYY') : null
    ...
    const onPress = useCallback(
        () => {
            navigation.push(moduleNames.NOTESDETAILS, {item: tmpItem})
        },
        [tmpItem],
    )
    ...
    const onDismiss = async (userIdA, key) => {
        const res = await Api.deleteNote(userIdA, key)
                                    ...
                                    <View style={{ width: '100%', height: '100%', overflow: 'hidden', alignItems: 'center', flexDirection: 'row', paddingRight: 16 }}>
                                        <View style={{ flex: 1, height: '100%', overflow: 'hidden', flexDirection: 'row', alignItems: 'center', /* paddingHorizontal: ConstNumbers.paddingHorizontalMain */ }}>
                                            ...
                                            <View style={{ height: '100%', flex: 1, justifyContent: 'center', alignItems: 'flex-start', }}>
                                                <Text style={[GlobalStyle.textFontRegular, { fontSize: 16, lineHeight: 24, color: theme.categoryTitleColor, }]} numberOfLines={1}>{hadithTitle}</Text>
                                                <Text style={[GlobalStyle.textFontRegular, { fontSize: 12, color: theme.singleHadithSubTitleColor, }]} numberOfLines={2}>{`${bookTitle}, ${hadithNumber} ${lastEdited ? '(ažurirano ' + lastEditedtext + ')' : ''}`}</Text>
                                            </View>
                                        </View>
                                    </View>
                                </SkeletonLoader>
