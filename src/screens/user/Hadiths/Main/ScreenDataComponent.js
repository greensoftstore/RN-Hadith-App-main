...
// Components
// ===================================================================
import { Header, ListEmptyComponent, SpinnerCustom, MainBackgroundWidthShadow, FullScreenLoader } from 'components'
...
function ScreenDataComponent({ navigation, screenSettings, selectedItem }) {
    ...
    const getData = useCallback(async () => {
        let res = await Api.getData(15, selectedItem, state.lastItem)
    ...
    const getMoreItems = useCallback(async () => {
        let res = await Api.getMoreData(15, selectedItem, state.lastItem)
    ...
    return (
        <>
            <Header
                title={selectedItem?.book?.bookTitle || ''}
                subtitle={selectedItem?.category?.categoryName || ''}
                navigation={navigation}
                displayBackButton={true}
            />

            <View style={{ width: '100%', flex: 1, padding: ConstNumbers.paddingHorizontalCard, }}>
                <MainBackgroundWidthShadow borderRadius={ConstNumbers.borderRadiusThird} >
                    ...
                    <View style={{ width: '100%', height: '100%', flex: 1, alignItems: 'center', overflow: 'hidden', borderRadius: ConstNumbers.borderRadiusMain, backgroundColor: theme.singleCollectionBackgroundColor, /* borderWidth: 1, borderColor: theme.singleCollectionBackgroundBorderColor, */ paddingBottom: 5 }} >
                        ...
                        <FlatList
                            ...
                            contentContainerStyle={{ minHeight: 180 }}
